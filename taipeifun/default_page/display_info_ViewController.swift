...
class display_info_ViewController: UIViewController {
    ...
    @IBAction func favorite_add(_ sender: Any) {
        ...
        if let appdelegate = UIApplication.shared.delegate as? AppDelegate{
            ...
            if (detail_array?.count)! > 0 && detail_array?.count != nil {
                favorite_exhibit.endTime = detail_array![0].endTime
            }
            else{
                 favorite_exhibit.endTime = "無相關資訊"
            }
...
//            favorite_exhibit.longtitude = detail_array![0].longitude
            if detail_array?.count != nil && (detail_array?.count)! > 0{
                ...
                favorite_exhibit.location = "無地址資訊"
                favorite_exhibit.locationName = "無所在地名稱資訊"
                favorite_exhibit.onSales = "無售票與否資訊"
                favorite_exhibit.price = "無票價資訊"
            }

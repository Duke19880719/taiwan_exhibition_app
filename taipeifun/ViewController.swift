...
class ViewController: UIViewController {
    ...
    var pass_data:pass_data_detail?
    ...
    func load_jsondata(url_path: String,change_word: String) {
        ...
        case "獨立音樂資訊":
            if independ_music_struct == nil
            {
                 alert_str = change_word
                 reduce(load_url: url_path)
                 independ_music_struct = self.data1
            }
            else
            {
...
extension ViewController:UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if data1![indexPath.row].imageUrl != "" {
        ...
        table_cell.location_sale.text = "\(location_str)   \(saleornot!)"
    ...
    func city_finf(location1:String) -> String  {
        ...
        {
            if   location1.hasPrefix("臺北市") || location1.hasPrefix("台北市")  {
                temp_str = "台北市"
            }
            if   location1.hasPrefix("臺中市") || location1.hasPrefix("台中市")  {
                temp_str = "台中市"
            }
            if   location1.hasPrefix("臺南市") || location1.hasPrefix("台南市")  {
                temp_str = "台南市"
            }
            if   location1.hasPrefix("臺東縣") || location1.hasPrefix("台東縣")  {
                temp_str = "台東縣"
            }
            if location1.hasPrefix(city[i]) {
...
//        let slideVC = destVC.topViewController as! ViewControllerSlide
        ...
        self.pass_data?.pass_data(data:data1![(tableview.indexPathForSelectedRow?.row)!].showInfo!)
       ...
       tran_obj.title_temp = "\(data1![(tableview.indexPathForSelectedRow?.row)!].title!)"
        ...
        var seprator = NSCharacterSet(charactersIn: "[]\"()中華民國")
